...
window.onload = function () {
  ...
  let pic = [];
  fetch(
    "https://opensheet.elk.sh/157VSYDe7ruS7q5wjSFDrFE_qsVQ1HKDkWoZ00dN096M/1 "
  ).then((res) => {
    if (res.status === 200) {
      // SUCCESS
      res
        ...
        .then((data) => {
          for (i = 0; i < data.length; i++) {
            ...
            item.push(data[i]["Item lost"]);
            pic.push(data[i]["Picture of item lost"]);
          }
          let html = "";
          for (i = 0; i < data.length; i++) {
            html += `
          ...
          if (html == "") {
            addhtml.innerHTML = `None of us Lost something :)`;
          } else {
